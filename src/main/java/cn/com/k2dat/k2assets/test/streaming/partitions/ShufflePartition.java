...
public class ShufflePartition {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<UP> dataStreamShuffle = dataStream.shuffle().map(new MapFunction<UP, UP>() {
            @Override
            public UP map(UP value) throws Exception {
                System.out.println("--shuffle: 当前线程=>" + Thread.currentThread().getId() + "; 输出内容 =>" + value.userId);
                return value;
            }
        }).setParallelism(4);
